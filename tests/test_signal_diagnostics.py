...
def _dense(
    ...
    **kwargs,
    ...
    if "outcome_scale" not in kwargs:
        kwargs["outcome_scale"] = np.ones(1, dtype=np.float32)
    return dense_signal_metrics(
        x, y, outcome, np.asarray(baseline, dtype=np.float32)[None], np.ones((1, 1)), **kwargs
    )
...
def _direct_only_edge_budget() -> dict[str, int | tuple[int, int]]:
    """Require one direct treatment and exclude upstream treatment parents."""
    return {
        "cy": (1, 1),
        "dc": 0,
        "dy": 0,
...
def test_numpy_carryover_matches_pytensor_mechanisms():
    x = np.array([0.2, 1.0, 0.5, 2.0, 0.7], dtype=np.float64)
    x_symbolic = pt.as_tensor_variable(x[:, None])
    geo = pytensor.function([], mechanisms.apply_geometric_carryover(x_symbolic, 0.6, 4))()[:, 0]
    weibull = pytensor.function(
        [], mechanisms.apply_weibull_pdf_carryover(x_symbolic, 3.0, 2.0, 4)
    )()[:, 0]
    assert np.allclose(_carryover_numpy(x, 1, 0.6, 1.0, 1.0, 4), geo)
    assert np.allclose(_carryover_numpy(x, 2, 0.0, 3.0, 2.0, 4), weibull)


def test_single_lag_weibull_is_identity_in_numpy_and_symbolic_paths():
    x = np.array([0.2, 1.0, 0.5], dtype=np.float64)
    with np.errstate(all="raise"):
        numpy_result = _carryover_numpy(x, 2, 0.0, 3.0, 2.0, 1)
        symbolic_result = pytensor.function(
            [],
            mechanisms.apply_weibull_pdf_carryover(pt.as_tensor_variable(x[:, None]), 3.0, 2.0, 1),
        )()[:, 0]
...
def test_degenerate_weibull_kernel_is_finite_and_consistent():
    ...
    shape = 2.0
    with np.errstate(all="raise"):
        numpy_result = _carryover_numpy(x, 2, 0.0, lam, shape, 2)
        symbolic_result = pytensor.function(
            [],
            mechanisms.apply_weibull_pdf_carryover(
                pt.as_tensor_variable(x[:, None]), lam, shape, 2
            ),
        )()[:, 0]
...
def test_weibull_guard_matches_numpy_across_compile_modes():
    ...
    for l_max in (4, 8):
        ...
        k_t = pt.dscalar("k")
        carryover = mechanisms.apply_weibull_pdf_carryover(
            pt.as_tensor_variable(x[:, None]), lam_t, k_t, l_max
        )
        compiled_carryover[l_max] = (
            pytensor.function([lam_t, k_t], carryover, mode="FAST_COMPILE"),
            pytensor.function([lam_t, k_t], carryover, mode="FAST_RUN"),
        )
    ...
    for l_max, lam, k in (
        ...
        fast_compile, fast_run = compiled_carryover[l_max]
        with np.errstate(all="ignore"):
            numpy_result = _carryover_numpy(x, 2, 0.0, lam, k, l_max)
...
def test_carryover_is_a_plain_normalized_causal_convolution():
    x = np.arange(1, 9, dtype=np.float64)
    result = _carryover_numpy(x, 1, 0.5, 1.0, 1.0, 3)
...
def test_spearman_uses_once_carryovered_observed_treatment_and_lmax_suffix():
    ...
    y = _carryover_numpy(x, 1, 0.55, 1.0, 1.0, 3).astype(np.float32)
    metrics, valid = _dense(
        x, y, l_max=3, carryover_family=np.array([[1]]), carryover_alpha=np.array([[0.55]])
    )
...
def test_flattened_signal_api_remains_compatible():
    ...
    flattened = per_treatment_signal(x, x, x[..., 0] + 1, np.ones((1, 1)), l_max=3)
    ...
    with_baseline = per_treatment_signal(
        x,
        x,
        x[..., 0] + 1,
        np.ones((1, 1)),
    ...
    assert summary["n_direct_treatments"] == 1
...
def test_r2_and_signed_baseline_correlation_contracts():
    ...
    treatment = np.ones((1, 6, 2), dtype=np.float32)
    contributions = np.zeros_like(treatment)
...
def test_r2_requires_residual_degrees_of_freedom_not_raw_column_count():
    ...
    treatment = np.ones((1, 3, 2), dtype=np.float32)
...
def test_short_constant_and_warmup_validity_contracts():
    ...
    assert short_valid[0, 0, _metric("treatment_hf")] == 1
    ...
    _, burned_valid = _dense(x, y, l_max=3, carryover_burn_in=3)
...
def test_summary_uses_valid_denominators_and_gate_contracts():
    ...
    assert not ok
    # Two direct treatments were eligible; the metric is missing because neither
    # pair produced a valid value, which the row must say instead of blaming
    # an absence of direct treatments.
    assert (
        "[FAIL] frac_spearman_lt_03 missing (no valid observations across 2 direct treatments)"
        in lines
    )
...
def test_empty_signal_summary_has_none_quantiles():
    ...
    assert summary["n_direct_treatments"] == 0
    assert all(value is None for value in summary["outcome_level_ratio_quantiles"].values())
...
def test_generated_shard_labels_match_loaded_array_recomputation(tmp_path):
    ...
    assert signal_config["carryover_kernel_semantics"] == "normalized-causal-minmax-weibull-density"
    ...
    metrics, valid = dense_signal_metrics(
        loaded["treatment_raw"],
        loaded["treatment_contribution_raw"],
        loaded["outcome_raw"],
        loaded["baseline_raw"],
        direct,
        outcome_scale=loaded["outcome_scale"],
        carryover_family=loaded["carryover_family"],
        carryover_alpha=loaded["carryover_alpha"],
        weibull_lam=loaded["weibull_lam"],
        weibull_k=loaded["weibull_k"],
        l_max=signal_config["l_max"],
        carryover_burn_in=signal_config["carryover_burn_in"],
    )
...
def test_validator_checks_signal_layout_dtype_and_eligibility():
    corpus = {
        "treatment_raw": np.ones((1, 4, 1), dtype=np.float32),
        "treatment_norm": np.ones((1, 4, 1), dtype=np.float32),
        "treatment_share": np.ones((1, 4, 1), dtype=np.float32),
        "covariates": np.ones((1, 4, 1), dtype=np.float32),
        "outcome_raw": np.ones((1, 4), dtype=np.float32),
        "outcome_norm": np.ones((1, 4), dtype=np.float32),
        "support_mask": np.array([[1, 1, 1, 0]], dtype=np.uint8),
        "is_future": np.zeros(1, dtype=np.uint8),
        "g": np.zeros((1, 6), dtype=np.uint8),
        "treatment_contribution_raw": np.zeros((1, 4, 1), dtype=np.float32),
        "baseline_raw": np.ones((1, 4), dtype=np.float32),
        "latent_unobserved": np.ones((1, 4, 1), dtype=np.float32),
        "treatment_means": np.ones((1, 1), dtype=np.float32),
        "outcome_scale": np.ones(1, dtype=np.float32),
        ...
        "treatment_active": np.zeros((1, 1), dtype=np.uint8),
        "covariate_contribution": np.zeros((1, 4, 1), dtype=np.float32),
        "latent_unobserved_contribution": np.zeros((1, 4, 1), dtype=np.float32),
        "baseline_intrinsic": np.ones((1, 4), dtype=np.float32),
        "outcome_noise": np.zeros((1, 4), dtype=np.float32),
        "indirect_effects_by_source": np.zeros((1, 4, 3), dtype=np.float32),
        "treatment_shock_mask": np.zeros((1, 4, 1), dtype=np.uint8),
        "treatment_shock_index": np.empty((1, 0), dtype=np.int32),
        "treatment_shock_start": np.empty((1, 0), dtype=np.int32),
        "treatment_shock_length": np.empty((1, 0), dtype=np.int32),
        "treatment_shock_level_multiplier": np.empty((1, 0), dtype=np.float32),
        "treatment_shock_level": np.empty((1, 0), dtype=np.float32),
        "treatment_level": np.ones((1, 1), dtype=np.float32),
        "saturation_scale": np.ones((1, 1), dtype=np.float32),
        "carryover_family": np.zeros((1, 1), dtype=np.uint8),
        "carryover_alpha": np.zeros((1, 1), dtype=np.float32),
...
def test_scale_free_metrics_match_their_unscaled_definitions():
    ...
    for key in ("treatment_cv", "contrib_cv"):
        index = _metric(key)
...
@pytest.mark.parametrize(
    ("kwargs", "match"),
    [
        ({"carryover_family": np.array([[1.9]])}, "carryover_family"),
        ({"carryover_family": np.array([[3]])}, "carryover_family"),
        (
            {
                "carryover_family": np.array([[1]]),
                "carryover_alpha": np.array([[np.nan]]),
            },
            "carryover_alpha",
        ),
        (
        ...
        (
            {
                "carryover_family": np.array([[1]]),
                "carryover_alpha": np.array([[1.1]]),
            },
            "carryover_alpha",
        ),
        (
            {
                "carryover_family": np.array([[2]]),
        ...
        ({"l_max": 0}, "l_max"),
        ({"l_max": 1.5}, "l_max"),
        ({"carryover_burn_in": -1}, "carryover_burn_in"),
        ({"carryover_burn_in": 0.5}, "carryover_burn_in"),
        ({"outcome_scale": np.array([np.nan])}, "outcome_scale"),
        ({"outcome_scale": np.array([0.0])}, "outcome_scale"),
        ({"outcome_scale": np.array([-1.0])}, "outcome_scale"),
    ],
)
def test_dense_signal_metrics_rejects_invalid_diagnostic_inputs(kwargs, match):
    base_kwargs = {"outcome_scale": np.ones(1)}
...
def test_summary_validates_outcome_and_outcome_scale():
    metrics = np.zeros((1, 1, len(SIGNAL_METRIC_LAYOUT)), dtype=np.float32)
    valid = np.zeros_like(metrics, dtype=np.uint8)
    mask = np.ones((1, 1), dtype=bool)
    with pytest.raises(ValueError, match="outcome"):
        summarize_signal_metrics(
            metrics, valid, np.array([[1.0, np.nan]]), mask, outcome_scale=np.ones(1)
        )
    with pytest.raises(ValueError, match="outcome_scale"):
        summarize_signal_metrics(metrics, valid, np.ones((1, 2)), mask, outcome_scale=np.zeros(1))
...
def test_response_support_weeks_tracks_the_realized_kernel_not_the_family():
    ...
    # A geometric treatment drawn at alpha == 0 is contemporaneous despite its
    ...
    zero_final_tap = _carryover_weights(2, 0.0, 3.0, 2.0, 8)
...
def test_admitted_response_support_weeks_bounds_every_draw_the_priors_allow():
    ...
    assert admitted_response_support_weeks([0], 8, carryover_alpha_range=(0.0, 0.8)) == 0
    assert admitted_response_support_weeks([1], 8, carryover_alpha_range=(0.0, 0.0)) == 0
    assert admitted_response_support_weeks([1], 8, carryover_alpha_range=(0.0, 0.8)) == 7
    # Weibull needs no decay range: its bound does not depend on one.
    assert admitted_response_support_weeks([2], 8, carryover_alpha_range=(0.0, 0.0)) == 7

    # It is a maximum over the treatments, and an empty treatment set or a
    # single-lag kernel admits no carryover at all.
    assert admitted_response_support_weeks([0, 2], 8, carryover_alpha_range=(0.0, 0.0)) == 7
    assert admitted_response_support_weeks([], 8, carryover_alpha_range=(0.0, 0.8)) == 0
    assert admitted_response_support_weeks([1, 2], 1, carryover_alpha_range=(0.0, 0.8)) == 0
    ...
    bound = admitted_response_support_weeks([1], 8, carryover_alpha_range=(0.0, 0.8))
    assert bound >= max(
        response_support_weeks(1, alpha, 1.0, 1.0, 8) for alpha in (0.0, 0.2, 0.5, 0.8)
    )

    with pytest.raises(ValueError, match="carryover_alpha_range"):
        admitted_response_support_weeks([1], 8, carryover_alpha_range=(0.8, 0.2))


def test_response_warmup_and_contemporaneous_diagnostics():
    metrics = np.zeros((1, 1, len(SIGNAL_METRIC_LAYOUT)), dtype=np.float32)
    valid = np.zeros_like(metrics, dtype=np.uint8)
    outcome = np.arange(8, dtype=float)[None]
    mask = np.ones((1, 1), dtype=bool)
    ...
    eligible_identity = summarize_signal_metrics(
        ...
        l_max=8,
        carryover_burn_in=8,
        carryover_family=np.array([[0, 2]], dtype=np.uint8),
    )
    ...
    with_burn_in = summarize_signal_metrics(
        ...
        l_max=8,
        carryover_burn_in=8,
        carryover_family=np.array([[2]], dtype=np.uint8),
        carryover_alpha=np.array([[0.0]], dtype=np.float32),
...
def test_response_warmup_weeks_uses_realized_reach_when_kernels_are_persisted():
    ...
    metrics = np.zeros((1, 1, len(SIGNAL_METRIC_LAYOUT)), dtype=np.float32)
    valid = np.zeros_like(metrics, dtype=np.uint8)
    outcome = np.arange(10, dtype=float)[None]
    mask = np.ones((1, 1), dtype=bool)
    ...
    def warmup(family: int, alpha: float, lam: float, k: float) -> int:
        return int(
            summarize_signal_metrics(
                ...
                l_max=8,
                carryover_burn_in=8,
                carryover_family=np.array([[family]], dtype=np.uint8),
                carryover_alpha=np.array([[alpha]], dtype=np.float32),
...
def test_response_warmup_matches_persisted_response_boundary():
    """Persisted treatment reproduces geometric response only after the reported boundary."""
    cfg = make_scm_prior(
        n_treatments=1,
        n_covariates=1,
        n_latent=1,
    ...
    assert np.all(corpus["carryover_family"][:, 0] == 1)

    for task in range(corpus["treatment_raw"].shape[0]):
        treatment = corpus["treatment_raw"][task, :, 0].astype(np.float64)
        contribution = corpus["treatment_contribution_raw"][task, :, 0].astype(np.float64)
        carryover = _carryover_numpy(
            treatment,
            int(corpus["carryover_family"][task, 0]),
            float(corpus["carryover_alpha"][task, 0]),
            float(corpus["weibull_lam"][task, 0]),
            float(corpus["weibull_k"][task, 0]),
            cfg.l_max,
        )
        response = carryover / float(corpus["saturation_scale"][task, 0])
...
def test_carryover_corpus_response_warmup_contrasts_and_self_validates():
    ...
    for name, family, family_probs in (
        ...
        assert np.all(corpus["carryover_family"] == family)
