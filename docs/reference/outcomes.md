...
drawn coefficients) or *signal* space (per-treatment CV, Spearman, warmup ratio).
...
media = dist["treatment_contribution"]   # one unit per (world, channel)
...
`baseline_intrinsic`, `outcome_noise`, `treatment_total_contribution`, `indirect_effects`)
and one `(world, column)` pair for a column quantity (`treatment_contribution`,
`covariate_contribution`, `latent_unobserved_contribution`, `indirect_by_source`,
`spend`, `controls`, `demand`).

Padded inactive columns are dropped, so no zero padding contaminates any
statistic. A structurally-null treatment — active, treatment observed, no `C→Y` edge
...
The quantities in `ADDITIVE_QUANTITIES` — `baseline_intrinsic`, `outcome_noise`,
`covariate_contribution`, `latent_unobserved_contribution`, `treatment_contribution`,
`indirect_by_source` — are exactly the per-node
[decomposition of outcome](../guide/decomposition.md). Their `unit_share` values
...
The one exception is a world whose outcome sum is exactly zero: its share budget
...
`outcome_raw` remains required for dimensions and shares. `outcome_scale` is required
only for `normalize="outcome_scale"`. The derived `treatment_total_contribution` additionally
needs `treatment_contribution_raw` and `indirect_effects`, but no per-column masks.
...
Worlds have arbitrary outcome levels, so pooled *raw* values mix scales.
`normalize="outcome_scale"` divides every Y-scale quantity by the world's own
`outcome_scale`; `normalize="outcome_mean"` divides by its mean outcome. Exogenous
inputs (`spend`, `controls`, `demand`) are not in outcome units and are never
