...
cells = [
    ...
    md(
        ...
        "\n"
        "The eight edge types are `cy` (channel→sales), `zb` (control→baseline),\n"
        "`db` (demand→baseline), `dc` (demand→channel), `dz` (demand→control),\n"
    ...
    md(
        ...
        "\n"
        "```\n"
        "sales = baseline_intrinsic\n"
        "      + Σ confounder_contribution      (D → B)\n"
        "      + Σ control_contribution         (Z → B)\n"
    ...
    md(
        ...
        "| --- | --- | --- |\n"
        "| **edge coefficients** | `beta` (`C→Y`), `rho_zb` (`Z→B`), `delta_db` (`D→B`), `w_dc` (`D→C`), `u_dz` (`D→Z`), `v_zc` (`Z→C`), `alpha_cc` (`C→C`), `gamma_zz` (`Z→Z`) | `*_coeff_range`, `beta_additive_range` |\n"
    ...
    md(
        ...
        "different ways:\n"
        "\n"
        "- **`zb`** — a control affects sales through the baseline. A regression that\n"
    ...
    code(
        ...
        "                 'db': (0, 0), 'dc': (0, 0), 'dz': (0, 0), 'zz': (0, 0)},\n"
        "    n_cells=2, draws_per_cell=1, seed=20260728,\n"
        "    **pin('geometric', 'michaelis_menten'),\n"
        ")\n"
        "mid = pg.sample_scm(interacting, seed=2, name='interacting')\n"
        "show(viz.plot_dag, mid, 'Controls feed the baseline AND spend; C->C interaction')"
    ),
    code(
    ...
    md(
        ...
        "the baseline (`db`). Now there is an unobserved common cause of treatment and\n"
        "outcome: a back-door path `C ← D → B → Y` that no regression on observables\n"
    ...
    code(
        ...
        "    db_coeff_range=(0.4, 0.8),\n"
        "    n_cells=2, draws_per_cell=1, seed=20260728,\n"
        "    **pin('geometric', 'michaelis_menten'),\n"
        ")\n"
        "confounded = pg.sample_scm(confounded_cfg, seed=2, name='confounded')\n"
        "show(viz.plot_dag, confounded, 'D drives spend AND the baseline: a hidden confounder')"
    ),
    code(
        ...
        "print('D -> B loading  (delta_db):', np.round(np.asarray(confounded.params['delta_db']), 3))\n"
        "print()\n"
    ...
    code(
        ...
        "      not any(np.asarray(rho_world.g[k]).any() for k in ('g_dc', 'g_db', 'g_dz')))"
    ),
    # ---------------------------------------------------------------- 9
    md(
    ...
    code(
        ...
        "    edge_budget={'cy': (2, 5), 'zb': (1, 3), 'dc': (0, 4), 'db': (0, 2),\n"
