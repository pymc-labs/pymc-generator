...
cells = [
    ...
    md(
        ...
        "Here `easy` means an estimable design, not merely a smooth line: 156 weeks, nine "
        "eight-week zero-spend interventions, one-lag geometric adstock (therefore no hidden "
        "carryover), and a nearly constant intrinsic baseline. The easy and difficult variants "
        "still differ only in their sales-noise scale."
    ),
    code(
        "EDGE_BUDGET = {\n"
        ...
        "}\n"
        "COMMON = dict(\n"
        "    n_treatments=3, n_covariates=2, n_latent=1, T=156,\n"
        "    l_max=1,\n"
        ...
        "    rw_baseline_std_sigma=0.005,\n"
        "    n_channel_shocks=9, channel_shock_length_range=(8, 8),\n"
        ")\n"
        "easy_cfg = pg.make_scm_prior(**COMMON, rw_sales_std_sigma=0.03)\n"
    ...
    code(
        "world = pg.sample_scm(easy_cfg, seed=5)\n"
        ...
        "shock_channels = set(np.asarray(world.params['channel_shock']['channel'], dtype=int))\n"
        "assert shock_channels == set(range(world.K))\n"
    ...
    code(
        ...
        "design = mmm_dataset[[*CHANNEL_COLUMNS, *CONTROL_COLUMNS]].iloc[RECOVERY_START:]\n"
        "design_corr = np.corrcoef(design.to_numpy().T)\n"
        "max_channel_control_corr = np.abs(design_corr[:world.K, world.K:]).max()\n"
        "print('max |channel-control correlation|:', f'{max_channel_control_corr:.3f}')\n"
        "assert max_channel_control_corr < 0.5\n"
    ...
    code(
        ...
        "            'relative_total_error': abs(median - true_total) / (abs(true_total) + 1e-12),\n"
        ...
        "recovery_pass = bool(\n"
        "    divergences == 0\n"
        "    and (channel_recovery['relative_total_error'] <= 0.1).all()\n"
        "    and (channel_recovery['curve_correlation'] >= 0.95).all()\n"
        "    and (channel_recovery['relative_curve_rmse'] <= 0.2).all()\n"
        ")\n"
        "print('max channel total error:', f\"{channel_recovery['relative_total_error'].max():.1%}\")\n"
        "print('channel-contribution recovery verdict:', 'PASS' if recovery_pass else 'FAIL')\n"
        "assert recovery_pass, recovery.round(3)"
    ),
    code(
    ...
    md(
        ...
        "The committed run returns `PASS`: all four chains complete with no divergences, every "
        "posterior-median cumulative channel contribution is within 10% of truth, and every "
        "weekly contribution path clears the correlation and normalized-RMSE gates printed "
        "above. The table also keeps the 94% interval indicator visible as a stricter uncertainty-"
        "calibration diagnostic; it is not substituted for the explicit practical recovery gate.\n\n"
        "This result depends on the stated easy-design contract. The graph satisfies back-door "
        "adjustment, every channel has observed zero-spend periods, the baseline drift is small, "
        "and one-lag adstock prevents unobserved carryover from crossing those interventions. "
        "A simple DAG alone would not guarantee finite-sample recovery; the overlap and noise "
        "conditions are part of what `easy` means here."
    ),
    md(
